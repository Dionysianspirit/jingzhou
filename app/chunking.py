...
Interview talking point: we split on blank lines first, pack paragraphs up to
CHUNK_CHARS, then hard-split leftover runs on '。' / '.' so a single giant
paragraph cannot blow the embedding window. Tiny tail fragments (<200 chars)
are merged backward to avoid orphan embeddings. Overlap keeps a definition
that sits on a boundary visible to both neighboring vectors.
"""
