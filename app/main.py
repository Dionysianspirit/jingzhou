...
async def mindmap(req: FeatureRequest):
    ...
    prompt = (
        ...
        "只输出 Markdown，不要其他文字。控制在15-25个节点。\n\n"
        f"文档内容：\n\n{ctx}"
    )
    try:
        resp = await llm.llm.chat.completions.create(
        ...
        try:
            stream = await llm.llm.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是学术写作助手。生成含摘要、核心概念、关键发现、总结的 Markdown 学习报告。",
                    },
                    {"role": "user", "content": f"文档内容：\n\n{ctx}\n\n请基于以上文档生成学习报告。"},
                ],
                stream=True,
...
async def study_guide(req: FeatureRequest):
    ...
    ctx = llm.build_ctx(results)
    prompt = (
        "你是课程助教。根据资料生成一份「学习指南」（NotebookLM Study Guide 风格）。\n"
        "只返回 JSON，字段：\n"
        '{"title":"标题","overview":"200字内导读","key_points":["要点"],'
        '"formulas":["公式或定义"],"pitfalls":["易错点"],'
        '"questions":[{"q":"思考题","hint":"提示"}],'
        '"next_steps":["下一步阅读建议"]}\n\n'
        f"文档内容：\n\n{ctx}"
    )
    try:
        ...
        raise HTTPException(500, "学习指南生成失败：模型返回格式异常")
    except HTTPException:
        raise
    except Exception as e:
...
async def infographic(req: FeatureRequest):
    ...
    ctx = llm.build_ctx(results)
    prompt = (
        "你是信息可视化专家。提取结构化数据，只返回 JSON：\n"
        '{"title":"...","stats":[{"label":"","value":"","desc":""}],'
        '"comparisons":[{"left":"","right":"","aspect":""}],'
        '"conceptFlow":[{"step":"","description":""}],'
        '"keyInsight":"..."}\n\n'
        f"文档内容：\n\n{ctx}"
    )
    try:
        return await _feature_json(prompt, 0.3, 2048)
    except json.JSONDecodeError:
        raise HTTPException(500, "信息图生成失败：模型返回格式异常")
    except HTTPException:
        raise
    except Exception as e:
