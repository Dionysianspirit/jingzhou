...
from app import agent, embeddings, features, llm
...
app = FastAPI(title="径舟", version="0.3.0", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def flashcards(req: FeatureRequest):
    ...
    try:
        return await features.feature_json(prompt, 0.5, 2048)
    except Exception as e:
...
async def quiz(req: FeatureRequest):
    _require_llm()
    try:
        questions = await features.gen_quiz(store, req.doc_ids, req.query, req.count)
        return {"questions": questions}
    except HTTPException as e:
        if e.status_code == 503:
            return {
                **features.SAMPLE_QUIZ,
                "fallback": True,
                "reason": "未配置 LLM，已载入示例考核",
            }
        raise
    except Exception as e:
...
async def study_guide(req: FeatureRequest):
    _require_llm()
    try:
        return await features.gen_guide(store, req.doc_ids, req.query)
    except Exception as e:
...
async def infographic(req: FeatureRequest):
    ...
    ctx = llm.build_ctx(results)
    try:
        return await features.feature_json(
            "提取 JSON stats/comparisons/conceptFlow/keyInsight\n\n" + ctx, 0.3, 2048
        )
    except Exception as e:
...
async def sample_quiz():
    return features.SAMPLE_QUIZ


@app.post("/api/agent")
async def agent_start(req: AgentStartRequest):
    _require_llm()
    mem = AgentMemory(goal=req.goal.strip(), doc_ids=req.doc_ids)
    agent_store.save(mem)
    return StreamingResponse(
        agent.run_session(mem, store, persist=agent_store),
        media_type="text/event-stream",
    )


@app.post("/api/agent/{session_id}/answers")
async def agent_answers(session_id: str, req: AgentAnswersRequest):
    _require_llm()
    mem = agent_store.load(session_id)
    if mem is None:
        raise HTTPException(404, "学习会话不存在")
    if mem.status != "awaiting_answers" or not mem.quiz:
        raise HTTPException(400, "该会话当前不在等待作答状态")
    if len(req.answers) != len(mem.quiz):
        raise HTTPException(
            400, f"答案数量不符：需要 {len(mem.quiz)} 份，收到 {len(req.answers)} 份"
        )
    mem.pending_answers = req.answers
    mem.status = "running"
    mem.stage = "判分"
    agent_store.save(mem)
    return StreamingResponse(
